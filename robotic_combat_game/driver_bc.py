...
def writeToTensorBoard(writer, tensorboardData, curr_episode):
    ...
    # writer.add_scalar(tag='Perf/Loss Rate', scalar_value=loss_rate, global_step=curr_episode)
        ...
        while True:
            ...
            if curr_episode % 1 == 0 and buffer_filled:
                ...
                for _ in range(8):
                    ...
                    with torch.no_grad():
                        ...
                        for i in range(N_SELF):
                            logs_ref = dp_policy_ref(self_node_inputs_batch, self_all_edge_inputs_batch[:, :, i*K_SIZE:(i+1)*K_SIZE], self_all_enemy_inputs_batch[:, :, i*N_ENEMY:(i+1)*N_ENEMY], \
                                                                self_all_current_inputs_batch[:, :, i].unsqueeze(2), self_node_padding_mask_batch, None, \
                                                                self_edge_mask_batch, None)
                    ...
                    logp_a = torch.gather(logp, 1, self_action_batch).squeeze(1).squeeze(1)

                    # 敌方
                    logps_list = []
                    for i in range(N_ENEMY):
                        logps_list.append(dp_policy_self(enemy_node_inputs_batch, enemy_all_edge_inputs_batch[:, :, i*K_SIZE:(i+1)*K_SIZE], enemy_all_enemy_inputs_batch[:, :, i*N_ENEMY:(i+1)*N_ENEMY], \
                                                         enemy_all_current_inputs_batch[:, :, i].unsqueeze(2), enemy_node_padding_mask_batch, None, \
                                                         enemy_edge_mask_batch, None))
                    logps = torch.stack(logps_list, dim=0)
                    ...
                    logp_b = torch.gather(logp, 1, enemy_action_batch).squeeze(1).squeeze(1)

                    policy_loss = - BETA * (logp_a + logp_b).mean()
                    
                    kl_div = ((logp_a-logp_ref) * logp_a.exp()).squeeze(2).sum(dim=-1).mean()
